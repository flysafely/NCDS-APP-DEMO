...
#define FARRADIUS 80.0f
#define STARTPOINT CGPointMake(30, 445)
    ...
    if (self) {
        ...
        _addButton = [[QuadCurveMenuItem alloc] initWithImage:[UIImage imageNamed:@"bg-addbutton.png"]
                                       highlightedImage:[UIImage imageNamed:@"bg-addbutton-highlighted.png"] 
                                           ContentImage:[UIImage imageNamed:@"detail_like_down"] 
                                highlightedContentImage:[UIImage imageNamed:@"detail_like_down"]];
...
- (void)quadCurveMenuItemTouchesBegan:(QuadCurveMenuItem *)item
{
    ...
    {
        self.expanding = !self.isExpanding;
        [MCSoundBoard playSoundForKey:@"socialitem"];
    }
}
- (void)quadCurveMenuItemTouchesEnd:(QuadCurveMenuItem *)item
{
    ...
    //判断执行函数
    if (item.tag==1000) {
        [MCSoundBoard playSoundForKey:@"socialplus"];
        [self.social sendtoWeibo];
    }else if (item.tag==1001){
        [MCSoundBoard playSoundForKey:@"socialplus"];
        [self.social sendtoFacebook];
    }else if (item.tag==1002){
        [MCSoundBoard playSoundForKey:@"socialplus"];
        [self.social sendtotwitter];
